/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TeamWebsiteWhereUniqueInput } from "./TeamWebsiteWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { TeamWebsiteUpdateInput } from "./TeamWebsiteUpdateInput";

@ArgsType()
class UpdateTeamWebsiteArgs {
  @ApiProperty({
    required: true,
    type: () => TeamWebsiteWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => TeamWebsiteWhereUniqueInput)
  @Field(() => TeamWebsiteWhereUniqueInput, { nullable: false })
  where!: TeamWebsiteWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => TeamWebsiteUpdateInput,
  })
  @ValidateNested()
  @Type(() => TeamWebsiteUpdateInput)
  @Field(() => TeamWebsiteUpdateInput, { nullable: false })
  data!: TeamWebsiteUpdateInput;
}

export { UpdateTeamWebsiteArgs as UpdateTeamWebsiteArgs };
