/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EventDatumWhereInput } from "./EventDatumWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EventDatumOrderByInput } from "./EventDatumOrderByInput";

@ArgsType()
class EventDatumFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EventDatumWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EventDatumWhereInput, { nullable: true })
  @Type(() => EventDatumWhereInput)
  where?: EventDatumWhereInput;

  @ApiProperty({
    required: false,
    type: [EventDatumOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EventDatumOrderByInput], { nullable: true })
  @Type(() => EventDatumOrderByInput)
  orderBy?: Array<EventDatumOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EventDatumFindManyArgs as EventDatumFindManyArgs };
