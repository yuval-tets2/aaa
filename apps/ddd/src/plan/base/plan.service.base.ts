...
export class PlanServiceBase {
  ...
  async count<T extends Prisma.PlanCountArgs>(
    args: Prisma.SelectSubset<T, Prisma.PlanCountArgs>
  ): Promise<number> {
  ...
  async findSemesters(
    parentId: string,
    args: Prisma.SemesterFindManyArgs
  ): Promise<Semester[]> {
  ...
  async getRequirements(parentId: string): Promise<DegreeRequirement | null> {
    return this.prisma.plan
      .findUnique({
        where: { id: parentId },
      })
