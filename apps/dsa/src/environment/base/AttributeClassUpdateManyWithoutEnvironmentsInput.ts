/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { AttributeClassWhereUniqueInput } from "../../attributeClass/base/AttributeClassWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class AttributeClassUpdateManyWithoutEnvironmentsInput {
  @Field(() => [AttributeClassWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [AttributeClassWhereUniqueInput],
  })
  connect?: Array<AttributeClassWhereUniqueInput>;

  @Field(() => [AttributeClassWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [AttributeClassWhereUniqueInput],
  })
  disconnect?: Array<AttributeClassWhereUniqueInput>;

  @Field(() => [AttributeClassWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [AttributeClassWhereUniqueInput],
  })
  set?: Array<AttributeClassWhereUniqueInput>;
}

export { AttributeClassUpdateManyWithoutEnvironmentsInput as AttributeClassUpdateManyWithoutEnvironmentsInput };
