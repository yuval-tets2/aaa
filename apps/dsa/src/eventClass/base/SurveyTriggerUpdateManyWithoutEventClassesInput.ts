/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SurveyTriggerWhereUniqueInput } from "../../surveyTrigger/base/SurveyTriggerWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class SurveyTriggerUpdateManyWithoutEventClassesInput {
  @Field(() => [SurveyTriggerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SurveyTriggerWhereUniqueInput],
  })
  connect?: Array<SurveyTriggerWhereUniqueInput>;

  @Field(() => [SurveyTriggerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SurveyTriggerWhereUniqueInput],
  })
  disconnect?: Array<SurveyTriggerWhereUniqueInput>;

  @Field(() => [SurveyTriggerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SurveyTriggerWhereUniqueInput],
  })
  set?: Array<SurveyTriggerWhereUniqueInput>;
}

export { SurveyTriggerUpdateManyWithoutEventClassesInput as SurveyTriggerUpdateManyWithoutEventClassesInput };
