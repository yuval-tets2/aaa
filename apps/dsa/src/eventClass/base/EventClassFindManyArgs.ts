/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EventClassWhereInput } from "./EventClassWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EventClassOrderByInput } from "./EventClassOrderByInput";

@ArgsType()
class EventClassFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EventClassWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EventClassWhereInput, { nullable: true })
  @Type(() => EventClassWhereInput)
  where?: EventClassWhereInput;

  @ApiProperty({
    required: false,
    type: [EventClassOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EventClassOrderByInput], { nullable: true })
  @Type(() => EventClassOrderByInput)
  orderBy?: Array<EventClassOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EventClassFindManyArgs as EventClassFindManyArgs };
