/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SurveyAttributeFilterWhereInput } from "./SurveyAttributeFilterWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SurveyAttributeFilterOrderByInput } from "./SurveyAttributeFilterOrderByInput";

@ArgsType()
class SurveyAttributeFilterFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SurveyAttributeFilterWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SurveyAttributeFilterWhereInput, { nullable: true })
  @Type(() => SurveyAttributeFilterWhereInput)
  where?: SurveyAttributeFilterWhereInput;

  @ApiProperty({
    required: false,
    type: [SurveyAttributeFilterOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SurveyAttributeFilterOrderByInput], { nullable: true })
  @Type(() => SurveyAttributeFilterOrderByInput)
  orderBy?: Array<SurveyAttributeFilterOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SurveyAttributeFilterFindManyArgs as SurveyAttributeFilterFindManyArgs };
